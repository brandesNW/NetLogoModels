...
to setup
  ...
  if not LevelSpace? [
    ...
    ; could there be lactoses inside?
  ]
  reset-ticks
end
...
; could use random with actual cell size info
...
to set-initial-variables [ the-list ]
  ...
  ; The list (as documented)
  ; cell-color
  ; initial-energy
  ; energy
  ; initial-LacYs
  ; initial-LacYs-inserted
  ; initial-LacZs
  ; initial-LacI-lactose-complexes
  ; initial-lactose-inside
  ; initial-lactose-outside
  ; glucose?-setting
  ; lactose?-setting
  ...
  ; energy   [ seems to be an error]
  ; 2        initial-LacYs
  ; 3        initial-LacYs-inserted
  ; 4        initial-LacZs
  ; 5        initial-LacI-lactose-complexes
  ; 6        initial-lactose-inside
  ; initial-lactose-outside   [ doesn't seem to be used ]
  ; 9        glucose?-setting [ order seems reversed]
  ; 8        lactose?-setting
